...
var Order = []string{
	...
	"netconf",
	// ipv6 идёт сразу за netconf, а не перед ним. Перечитывая свои файлы,
	// systemd-networkd переписывает у каждого управляемого линка
	// net.ipv6.conf.<линк>.accept_ra: приём объявлений маршрутизатора он
	// делает сам и ядру его не оставляет. Пока ipv6 отрабатывал раньше, режим
	// passthrough не включался вовсе — проверка после применения находила на
	// аплинке 0 вместо 1 и откатывала конфигурацию целиком. Последнее слово о
	// параметрах ядра принадлежит netOS, поэтому он говорит его последним.
	"ipv6",
	"interfaces",
	"networks",
	"wan",

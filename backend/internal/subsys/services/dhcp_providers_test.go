...
func TestKeaDHCPRender(t *testing.T) {
	...
	for _, want := range []string{`"interfaces"`, `"br0"`, `"service-sockets-max-retries": 60`, `"service-sockets-retry-wait-time": 1000`, `"192.168.50.0/24"`, `"DROP"`, `"192.168.50.10"`, `"` + strings.ReplaceAll(keaLeasePath, `\`, `\\`) + `"`} {
		if !strings.Contains(out, want) {

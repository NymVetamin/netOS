...
func renderPassive(iface config.Interface, suppressIPv6 bool) string {
	...
	// В режиме прямого управления мосты, агрегации и их состав строит netOS
	// командами ip, и networkd о них не знает: файлов .netdev здесь нет.
	// Без KeepMaster networkd считает отсутствие Bridge= указанием вывести
	// порт из моста, и делает это при каждом networkctl reload — то есть при
	// любом применении конфигурации, задевшем файлы. Проводной сегмент
	// оставался с одним лишь Wi-Fi в мосту, а проверка после применения
	// сообщала, что порт не включён в мост; откат чинил не больше, чем
	// применение, потому что перечитывал те же файлы.
	w("KeepMaster=yes")
	if suppressIPv6 {
